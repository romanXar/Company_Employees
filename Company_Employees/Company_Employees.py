...
class Main(tk.Frame):
    ...
    def update_record(self,name,tel,email,salary):
        self.db.c.execute("""
        UPDATE db SET name = ?,tel = ?,email = ?,salary = ?
        WHERE ID = ?""",(name,tel,email,salary,self.tree.set(
    ...
    def init_main(self):
        ...
        btn_open_dialog = tk.Button(toolbar, bg='#d7d8e0', bd=1, image = self.add_img, command=self.open_dialog)
        btn_open_dialog.pack(side=tk.LEFT)

        # кнопка изменения данных
        self.update_img = tk.PhotoImage(file='./img/update.png')
        btn_edit_dialog = tk.Button(toolbar,bg = '#d7d8e0',bd = 1,image =self.update_img,command = self.open_update_dialog)
        btn_edit_dialog.pack(side=tk.LEFT)

        # кнопка удаления данных
        self.delete_img = tk.PhotoImage(file = './img/delete.png')
        btn_delete = tk.Button(toolbar,bg = '#d7d8e0',bd=1,image = self.delete_img,command = self.delete_records)
        btn_delete.pack(side = tk.LEFT)
        
        # кнопка поиска данных
        self.search_img = tk.PhotoImage(file = './img/search.png')
        btn_search = tk.Button(toolbar,bg = '#d7d8e0',bd=1,image = self.search_img,command = self.open_search_dialog)
        btn_search.pack(side = tk.LEFT)
        
        # кнопка обновления
        self.upd_view_img = tk.PhotoImage(file = './img/update_view.png')
        btn_upd_view = tk.Button(toolbar,bg = '#d7d8e0',bd=1,image = self.upd_view_img,command = self.view_records)
        btn_upd_view.pack(side = tk.LEFT)
...
class Update(Child):
    def __init__(self):
        ...
        if len(app.tree.selection()) == 0:
             messagebox.showerror("Oшибка!"," Вы не выбрали сотрудника!")
             return
        else:
             super().__init__()
             self.init_edit() 
